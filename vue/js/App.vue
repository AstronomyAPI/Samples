<template>
  <div class="container">
    <h2><img
      ...
      Astronomy API - Demo</h2>

    <div class="row">
      <div class="column column-25">
        <p>
          Copy your App Id and the App Secret from the
          <a href="https://astronomyapi.com/dashboard" target="_blank">Astronomy API console</a>
          paste below to see the samples in action.
        </p>
        <form>
          <fieldset>
            <label>App Id</label>
            <input v-model="store.appId" @change="validateCredentials" type="text" />
            <label>App Secret</label>
            <input v-model="store.appSecret" @change="validateCredentials" type="password" />
          </fieldset>
        </form>

        <div v-bind:class="messageClass" v-if="status">{{ status }}</div>

        <p v-if="store.credentialsValid">
          Explore each endpoint below.
        </p>
...
export default {
  ...
  methods: {
    validateCredentials() {
      ...
      axios
        .get(
          `${store.apiEndpoint}/api/v2/bodies`,
          {
            headers: {
